...
let version = core.getInput("aliyun-cli-version");
...
async function findLatestRelease() {
  ...
  return result.data.tag_name.slice(1);
}

async function run() {
  if (!version) {
    version = await findLatestRelease();
  }
  ...
  if ("win32" === platform) {
    fs.copyFileSync(
      path.normalize(`${extractedPath}/aliyun.exe`),
      path.normalize(`${extractedPath}/aliyun`)
    );
  }
